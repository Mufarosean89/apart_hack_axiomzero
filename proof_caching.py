...
class ProofCache:
    ...
    def add_proof(self, proof: CachedProof):
        ...
        logger.info(
            f"Cached proof: {proof.theorem_id} ({proof.proof_length} tactics)")
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        avg_length = sum(
            p.proof_length for p in self.cache.values()) / len(self.cache)
        avg_time = sum(p.search_time for p in self.cache.values()
                       ) / len(self.cache)
...
class TransferLearningAgent:
    ...
    def initialize_search(self, theorem_statement: str) -> Dict[str, Any]:
        ...
        # Aggregate tactics
        suggested_tactics = self.cache.suggest_tactics(
            theorem_statement, top_k=10)
    ...
    def adapt_strategy(
        ...
        if current_depth > 10:
            ...
            if short_proofs:
                ...
                valid_suggestions = list(dict.fromkeys(
                    short_tactics))  # Unique, preserve order
...
if __name__ == "__main__":
    ...
    for theorem in theorems:
        ...
        print(
            f"✓ {theorem['id']}: {result['source']} ({result.get('proof_length', 0)} tactics)")
