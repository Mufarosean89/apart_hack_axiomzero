...
class BenchmarkEvaluator:
    ...
    def compute_summary(self, total_time: float) -> Dict[str, Any]:
        ...
        if successful:
            avg_proof_length = sum(r['proof_length']
                                   for r in successful) / len(successful)
            avg_search_time = sum(r['search_time']
                                  for r in successful) / len(successful)
            avg_mcts_sims = sum(r['mcts_simulations']
                                for r in successful) / len(successful)
        else:
            avg_proof_length = 0
            avg_search_time = 0
    ...
    def print_results(self, summary: Dict[str, Any]):
        ...
        print(
            f"  Benchmarks: {summary['successful']}/{summary['total_benchmarks']}")
        ...
        for level, stats in sorted(summary['by_level'].items()):
            print(
                f"  {level:<10} {level_names[level]:<25} {stats['success_rate']:.1%}")
        ...
        for diff, stats in sorted(summary['by_difficulty'].items()):
            print(
                f"  {diff:<15} {stats['success_rate']:.1%} {stats['total']:<10}")
        ...
        print(
            f"  Wasted Simulations: {summary['total_wasted_simulations']:.0f}")

        # Individual results
        print(f"\nDetailed Results:")
        print(
            f"  {'Benchmark':<25} {'Level':<6} {'Result':<8} {'Steps':<6} {'Time':<8}")
        print(f"  {'-'*53}")
...
def compare_with_baseline():
    ...
    print(
        f"  Avg Time (success): {sum(r['time'] for r in baseline.values() if r['success']) / baseline_success:.2f}s")
