...
def main():
    ...
    print(
        f"  Levels completed: {curriculum_summary['current_level']}/{curriculum_summary['total_levels']}")
    ...
    print(
        f"  Benchmarks solved: {eval_summary['successful']}/{eval_summary['total_benchmarks']}")
    ...
    for diff in ['easy', 'medium', 'hard']:
        if diff in eval_summary['by_difficulty']:
            ...
            print(
                f"    {diff:10s}: {stats['success_rate']:.1%} ({stats['successful']}/{stats['total']})")

    print()
    print(f"  Proof Quality:")
    print(
        f"    Avg Proof Length: {eval_summary['avg_proof_length']:.1f} tactics")
    ...
    easy_rate = eval_summary['by_difficulty'].get(
        'easy', {}).get('success_rate', 0)
    medium_rate = eval_summary['by_difficulty'].get(
        'medium', {}).get('success_rate', 0)
    hard_rate = eval_summary['by_difficulty'].get(
        'hard', {}).get('success_rate', 0)
    ...
    print("Summary:")
    print(
        f"  ✓ Curriculum learning: {curriculum_summary['current_level']}/{curriculum_summary['total_levels']} levels")
    print(
        f"  ✓ Benchmark evaluation: {eval_summary['success_rate']:.1%} success rate")
